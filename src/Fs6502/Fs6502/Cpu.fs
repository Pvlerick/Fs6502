namespace Fs6502
    ...
    module Cpu =
        ...
        type Cpu() = 
            ...
            member val public Status = [| false; false; true; false; false; false; false; false |]
            
            member val private Memory = Array.create 1024 0uy
            ...
            //Parse operands to find the addressing mode
//            member this.GetAddressingMode(operand)
//                match operand with
//                | 
...
//                | TYA args -> this.TYA(args)
            ...
            member this.ParseOperand(operand) =
                ...
                match operand with
                | RegexGroups "#\$(\d{2})" args -> Mode.Immediate(args)
                | RegexGroups "\$(\d{2})" args -> Mode.ZeroPage(args)
                | RegexGroups "\$(\d{2}),X" args -> Mode.ZeroPageX(args)
                | RegexGroups "\$(\d{2}\d{2})" args -> Mode.Absolute(args)
                | RegexGroups "\$(\d{2}\d{2}),X" args -> Mode.AbsoluteX(args)
                | RegexGroups "\$(\d{2}\d{2}),Y" args -> Mode.AbsoluteY(args)
                | RegexGroups "\(\$(\d{2}),X\)" args -> Mode.IndirectX(args)
                | RegexGroups "\(\$(\d{2})\),Y" args -> Mode.IndirectX(args)
...
//                else None
            ...
            member private this.LDA mode = 
                match mode with
                | Mode.Immediate(Arguments.One(arg)) -> this.Accumulator <- arg
